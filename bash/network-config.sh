...
#HOSTNAME
dataforhost=$(hostname)
	echo "Hostname      : $dataforhost"

#LAN ADDRESS
dataforip=$(ip a s ens33 | 
	grep -w inet | 
	awk '{print $2}')
	echo "LAN Address   : $dataforip"

#LAN HOSTNAME
getlan=$(getent hosts $(hostname -I)| 
	awk '{print $2}')
	echo "LAN Hostname  : $getlan"

#EXTERNAL IP
extrnlip=$(curl -s icanhazip.com)
	echo "External IP   : $extrnlip"

#EXTERNAL NAME

extrnlname=$(getent hosts $(curl -s icanhazip.com) | 
	awk '{print $2}')
	echo "External Name : $extrnlname"
	
#ROUTER ADDRESS
routeraddr=$(ip route |  awk '{print $3}')
	echo "Router Address: $routeraddr"
